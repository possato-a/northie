...
// ─── Agent ────────────────────────────────────────────────────────────────────
export async function runStrategicAdvisor(client, businessType, trafficAnalysis, conversionAnalysis, attributionAnalysis) {
...
Com base nos três diagnósticos e no perfil do founder, gere o diagnóstico estratégico executivo completo em JSON.`;
    ...
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('[StrategicAdvisorAgent] Timeout após 30s')), TIMEOUT_MS));
    let rawText;
    try {
        rawText = await Promise.race([callApi(), timeout]);
    }
    catch (err) {
        throw new Error(`[StrategicAdvisorAgent] Falha na chamada à API: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
        ...
        throw new Error(`[StrategicAdvisorAgent] Falha ao parsear JSON: ${match[0].slice(0, 120)}`);
    }
}
//# sourceMappingURL=strategic-advisor.agent.js.map