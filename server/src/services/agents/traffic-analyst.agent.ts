...
export async function runTrafficAnalyst(
    client: Anthropic,
    metrics: AdMetricRow[],
...
Analise os dados acima e retorne o JSON de diagnóstico de tráfego.`;
    ...
    const timeout = new Promise<never>((_, reject) =>
        setTimeout(
            () => reject(new Error('[TrafficAnalystAgent] Timeout após 30s')),
            TIMEOUT_MS,
        ),
    );
    ...
    try {
        ...
        throw new Error(
            `[TrafficAnalystAgent] Falha na chamada à API: ${err instanceof Error ? err.message : String(err)}`,
        );
    }
    ...
    try {
        return JSON.parse(match[0]) as TrafficAnalysis;
    } catch {
        throw new Error(
            `[TrafficAnalystAgent] Falha ao parsear JSON: ${match[0].slice(0, 120)}`,
        );
    }
}
