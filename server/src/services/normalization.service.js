...
async function handleStripeNormalization(payload, profileId) {
    if (payload.type === 'checkout.session.completed') {
        ...
        if (!email)
            throw new Error('Customer email missing in Stripe session');
...
async function handleKiwifyNormalization(payload, profileId) {
 ...
 */
async function syncTransaction(profileId, email, platform, externalId, amount, visitorId) {
