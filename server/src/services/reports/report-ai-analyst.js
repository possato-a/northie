...
// ── Formatters ────────────────────────────────────────────────────────────────
const fmt = (n, decimals = 2) => n.toLocaleString('pt-BR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
...
function parseAnalysis(raw) {
    ...
    const diagnosticos = Array.isArray(parsed.diagnosticos)
        ? parsed.diagnosticos
            ...
            severidade: validSeveridade.includes(d.severidade) ? d.severidade : 'media',
            ...
            prazo: validPrazo.includes(d.prazo) ? d.prazo : 'este_mes',
        }))
        : [];
    return {
...
export async function* streamReportNarrative(data, signal) {
