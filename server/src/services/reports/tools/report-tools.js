...
export async function getHistoricalSnapshots(profileId, months = 6) {
...
export async function calculateChannelCacTrend(profileId, months = 4) {
    const since = new Date();
    since.setMonth(since.getMonth() - months);
    ...
    for (const a of (adData ?? [])) {
        ...
        if (!spendByChannelMonth[ch])
            spendByChannelMonth[ch] = {};
...
export async function getChurnRiskByChannel(profileId) {
    const { data } = await supabase
        .from('customers')
    ...
    for (const c of data) {
        const ch = c.acquisition_channel ?? 'desconhecido';
        if (!byChannel[ch])
            byChannel[ch] = { total: 0, probSum: 0, highRisk: 0, highRiskLtv: 0 };
        const prob = Number(c.churn_probability ?? 0);
        const ltv = Number(c.total_ltv ?? 0);
        byChannel[ch].total++;
        byChannel[ch].probSum += prob;
        if (prob > 0.6) {
            byChannel[ch].highRisk++;
            byChannel[ch].highRiskLtv += ltv;
        }
    }
...
export async function getCohortRepeatPurchase(profileId, months = 4) {
    const since = new Date();
    since.setMonth(since.getMonth() - months);
    ...
    for (const c of custData) {
        ...
        if (!groups[key])
            groups[key] = { total: 0, repeat: 0 };
        groups[key].total++;
        if ((txCountByCust[c.id] ?? 0) > 1)
            groups[key].repeat++;
    }
...
// ── Dispatcher ────────────────────────────────────────────────────────────────
export async function executeTool(name, input, profileId) {
    try {
        switch (name) {
