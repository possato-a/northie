...
export function identifyBusinessModel(revenueByPlatform) {
...
export function consolidateRevenue(revenueByPlatform) {
    const total = Object.values(revenueByPlatform).reduce((s, v) => s + v, 0);
...
function scoreComponent(v, excellent, healthy, attention, higherBetter) {
    if (higherBetter) {
        if (v >= excellent)
            return 100;
        if (v >= healthy)
...
export function computeHealthScore(input) {
    ...
    const bl = (s) => s >= 75 ? 'Otimo' : s >= 50 ? 'Regular' : s >= 25 ? 'Fraco' : 'Critico';
    return {
        score, label, color,
        breakdown: {
...
export function computeProjections(revenueTrend, currentRevenue, periodDays) {
...
export function detectMissingIntegrations(revenueByPlatform, spendByPlatform) {
    ...
    return checks.filter(c => {
        ...
        if (hasAnySalesData && !hasAnyAdsData && (c.keys.includes('meta') || c.keys.includes('google')))
            return true;
        // Mostrar plataformas de vendas faltantes como sugestao
        if (c.keys.includes('stripe') || c.keys.includes('hotmart') || c.keys.includes('shopify'))
            return true;
        return false;
...
//# sourceMappingURL=report-business-model.js.map