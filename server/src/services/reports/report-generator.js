...
export async function generateReportData(profileId, frequency) {
    ...
    const [{ data: transactions }, { data: adMetrics }, { data: newCustomers }, { data: topCustomers }, { data: prevTransactions }, { data: atRiskCustomers }, { data: rfmBase },] = await Promise.all([
        supabase
            .from('transactions')
    ...
    // ── RFM distribution ──────────────────────────────────────────────────────
    const rfmDistribution = (rfmBase ?? []).length > 0
        ? categorizeRfm(rfmBase)
        : [];
...
export function formatAsCsv(data, ai) {
    const { period, summary, revenue_by_platform, spend_by_platform, channel_economics, rfm_distribution, at_risk_customers, } = data;
