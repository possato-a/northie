...
// ── Row writers ───────────────────────────────────────────────────────────────
function writeSectionTitle(ws, rowNum, text) {
...
function writePeriodRow(ws, rowNum, text) {
    ws.mergeCells(rowNum, 1, rowNum, HC);
    const cell = ws.getCell(rowNum, 1);
    cell.value = `   ${text}`;
...
function writeSectionDivider(ws, rowNum, label) {
    ws.mergeCells(rowNum, 1, rowNum, HC);
    const cell = ws.getCell(rowNum, 1);
    cell.value = `  ▸  ${label}`;
...
function applyOuterBorder(ws, startRow, endRow, startCol, endCol) {
    const side = { style: 'thin', color: { argb: 'FFDEDBD5' } };
...
function setWsDefaults(ws, ySplit) {
