...
function hasAnyData(data: ReportData): boolean {
...
function drawNoIntegrationsPage(doc: PDFDoc, data: ReportData): void {
    const profileName = data.profile_name ?? 'Seu Negocio';
    const today = new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
    ...
    doc.font('Helvetica').fontSize(9).fillColor('#92400E')
        .text('Conecte ao menos uma integracao para ver sua analise completa.',
            MARGIN + 16, doc.y + 26, { width: CONTENT_W - 32 });
    doc.y += 68;
    ...
    doc.font('Helvetica').fontSize(9).fillColor(C.textSecondary)
        .text(
            'Acesse app.northie.com.br → Integracoes para conectar suas plataformas. ' +
            'Apos a conexao, seu proximo relatorio tera analise completa de receita, ' +
            'LTV, CAC, ROAS e saude do negocio.',
