...
function threeColTable(
    ...
    colBLabel: string,
    colCLabel: string,
    colCColor?: (val: string) => string | undefined,
) {
...
function channelEconTable(
    doc: PDFDoc,
    channels: Awaited<ReturnType<typeof import('./report-generator.js').generateReportData>>['channel_economics'],
) {
    ...
    channels.forEach((ch, idx) => {
        ...
        doc.font('Helvetica').fontSize(8).fillColor(C.dark)
            .text(ch.channel, cx, rowY + 4, { width: cW[0]! - 6 }); cx += cW[0]!;
        doc.font('Courier').fontSize(8)
            .text(String(ch.new_customers), cx, rowY + 4, { width: cW[1]! - 6, align: 'right' }); cx += cW[1]!;
        doc.font('Courier').fontSize(8)
            .text(fmtBrl(ch.avg_ltv), cx, rowY + 4, { width: cW[2]! - 6, align: 'right' }); cx += cW[2]!;
        doc.font('Courier').fontSize(8)
            .text(cacStr, cx, rowY + 4, { width: cW[3]! - 6, align: 'right' }); cx += cW[3]!;
        doc.font('Courier').fontSize(8)
            .text(ltvcacStr, cx, rowY + 4, { width: cW[4]! - 6, align: 'right' }); cx += cW[4]!;
        doc.font('Courier').fontSize(8).fillColor(valorColor)
            .text(fmtBrl(ch.value_created), cx, rowY + 4, { width: cW[5]! - 6, align: 'right' }); cx += cW[5]!;
...
export async function generatePdf(
    ...
    if (criticalDiags.length > 0) {
        ...
        for (const d of criticalDiags) {
            if (doc.y > 680) break; // Página 1 tem limite
            const color = SEVERITY_COLOR[d.severidade];
            const y = doc.y;
            doc.save()
                .rect(MARGIN, y, 4, 28)
    ...
    if (data.revenue_trend.length >= 2) {
        ...
        const trendRows = data.revenue_trend.map((t): [string, string, string] => {
    ...
    if (data.top_products.length > 0) {
        ...
        const productRows = data.top_products.map((p): [string, string, string] => [
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc.font('Helvetica')
            ...
            .fillColor(C.textSecondary)
            .text(
                `Gerado em ${fmtDate(ai.generated_at)} · Análise por ${ai.model} · Northie · Pág. ${pageNum}/${totalPages}`,
                MARGIN,
                footerY,
