...
// ── Drawing primitives ─────────────────────────────────────────────────────────
function drawHRule(doc, y, color = C.border, width = 0.5) {
    doc.save()
...
function drawSectionHeader(doc, num, title, profileName, periodLabel, forceNewPage = true) {
    if (forceNewPage) {
        doc.addPage();
        drawPageHeader(doc, profileName, periodLabel);
        doc.y = HEADER_H + 16;
...
function drawSubLabel(doc, title) {
...
function drawKpiGrid(doc, cards, cols = 3) {
    const gap = 8;
...
// ── Health score bar ───────────────────────────────────────────────────────────
function drawHealthScore(doc, score, label, color, breakdown) {
    ...
    bKeys.forEach(([, v], i) => {
        ...
        doc.font('Helvetica').fontSize(7).fillColor(C.textSecondary)
            .text(['LTV/CAC', 'Reembolso', 'Crescimento', 'ROAS'][i] ?? '', bx, by, { width: bColW - 4 });
...
// ── Cover page ─────────────────────────────────────────────────────────────────
function drawCoverPage(doc, data, ai) {
    const coverH = 310;
    ...
    doc.font('Helvetica').fontSize(7.5).fillColor(C.textSecondary)
        .text('Este relatorio e confidencial e destinado exclusivamente ao proprietario do negocio.', MARGIN, PAGE_H - 55, { width: CONTENT_W, align: 'center' });
    doc.font('Helvetica').fontSize(7.5).fillColor(C.textSecondary)
        .text('Gerado pela Northie | northie.com.br', MARGIN, PAGE_H - 42, { width: CONTENT_W, align: 'center' });
}
// ── Secao 1 — Resumo Executivo ─────────────────────────────────────────────────
function drawSection1(doc, data, ai, profileName, periodLabel) {
    drawSectionHeader(doc, '01', 'RESUMO EXECUTIVO', profileName, periodLabel);
    // Health score
    if (data.health_score) {
        drawSubLabel(doc, 'Saude do Negocio');
        drawHealthScore(doc, data.health_score.score, data.health_score.label, data.health_score.color, data.health_score.breakdown);
        doc.moveDown(0.4);
    }
...
function drawSection2(doc, data, profileName, periodLabel) {
    ...
    if (cr && cr.is_multi_source && cr.by_source.length > 0) {
        ...
        const cols = [
            { label: 'PLATAFORMA', width: 0.30, align: 'left' },
            { label: 'RECEITA', width: 0.30, align: 'right', font: 'Courier' },
            { label: '% DO TOTAL', width: 0.20, align: 'right', font: 'Courier' },
            { label: 'MODELO', width: 0.20, align: 'left' },
        ];
        const modelMap = { stripe: 'SaaS', hotmart: 'Infoproduto', shopify: 'E-commerce' };
    ...
    if (data.revenue_trend.length >= 2) {
        ...
        const trendCols = [
            { label: 'MES', width: 0.18, align: 'left' },
            { label: 'RECEITA', width: 0.30, align: 'right', font: 'Courier' },
            { label: 'VARIACAO', width: 0.20, align: 'right', font: 'Courier' },
            { label: 'TENDENCIA', width: 0.32, align: 'left' },
        ];
    ...
    if (data.top_products.length > 0) {
        ...
        const prodCols = [
            { label: 'PRODUTO', width: 0.40, align: 'left' },
            { label: 'RECEITA', width: 0.25, align: 'right', font: 'Courier' },
            { label: 'TRANSACOES', width: 0.18, align: 'right', font: 'Courier' },
            { label: '% DO TOTAL', width: 0.17, align: 'right', font: 'Courier' },
        ];
...
function drawSection3(doc, data, profileName, periodLabel) {
    ...
    if (!hasAds) {
        doc.font('Helvetica').fontSize(9.5).fillColor(C.textSecondary)
            .text('Nenhum dado de investimento em ads encontrado neste periodo. Conecte Meta Ads ou Google Ads para visualizar esta secao.', MARGIN, doc.y, { width: CONTENT_W });
        doc.moveDown(1);
        return;
    }
    // Total ads overview
    ...
    if (spendPlatforms.length > 0) {
        ...
        const adCols = [
            { label: 'PLATAFORMA', width: 0.22, align: 'left' },
            { label: 'INVESTIMENTO', width: 0.22, align: 'right', font: 'Courier' },
            { label: 'RECEITA ATRIBUIDA', width: 0.24, align: 'right', font: 'Courier' },
            { label: 'ROAS REAL', width: 0.16, align: 'right', font: 'Courier' },
            { label: 'STATUS', width: 0.16, align: 'center' },
        ];
    ...
    if (channels.length > 0) {
        ...
        const econCols = [
            ...
            { label: 'NOVOS CLI.', width: 0.12, align: 'right', font: 'Courier' },
            { label: 'LTV MEDIO', width: 0.18, align: 'right', font: 'Courier' },
            { label: 'CAC', width: 0.16, align: 'right', font: 'Courier' },
            { label: 'LTV/CAC', width: 0.13, align: 'right', font: 'Courier' },
            { label: 'VALOR CRIADO', width: 0.23, align: 'right', font: 'Courier' },
        ];
...
function drawSection4(doc, data, profileName, periodLabel) {
    ...
    if (data.at_risk_customers.length > 0) {
        ...
        const riskCols = [
            { label: 'LTV', width: 0.22, align: 'right', font: 'Courier' },
            { label: 'CANAL', width: 0.20, align: 'left' },
            { label: 'PROB. CHURN', width: 0.18, align: 'right', font: 'Courier' },
            { label: 'DIAS S/ COMPRA', width: 0.20, align: 'right', font: 'Courier' },
            { label: 'RFM SCORE', width: 0.20, align: 'center' },
        ];
    ...
    if (rfmFiltered.length > 0) {
        ...
        const rfmCols = [
            { label: 'SEGMENTO', width: 0.18, align: 'left' },
            { label: 'CLIENTES', width: 0.14, align: 'right', font: 'Courier' },
            { label: 'LTV TOTAL', width: 0.22, align: 'right', font: 'Courier' },
            { label: 'DESCRICAO', width: 0.46, align: 'left' },
        ];
...
function drawSection5(doc, data, profileName, periodLabel) {
    ...
    doc.font('Helvetica').fontSize(9).fillColor(C.textSecondary)
        .text('Esta secao consolida proxies de engajamento disponiveis. Conecte as integracoes abaixo para dados completos.', MARGIN, doc.y, { width: CONTENT_W });
    doc.moveDown(0.8);
...
// ── Secao 6 — Projecoes ────────────────────────────────────────────────────────
function drawSection6(doc, data, profileName, periodLabel) {
    drawSectionHeader(doc, '06', 'PROJECOES — PROXIMOS 3 MESES', profileName, periodLabel);
    const proj = data.projections;
    if (!proj) {
        doc.font('Helvetica').fontSize(9).fillColor(C.textSecondary)
            .text('Dados insuficientes para gerar projecoes. Necessario pelo menos 2 meses de historico.', MARGIN, doc.y, { width: CONTENT_W });
        return;
    }
    // Trajectory note
    doc.font('Helvetica').fontSize(9.5).fillColor(C.dark)
    ...
    doc.font('Helvetica').fontSize(8).fillColor(C.textSecondary)
        .text(`Base mensal atual: ${fmtBrl(proj.base_monthly)} | Tendencia recente: ${proj.trend_rate_pct >= 0 ? '+' : ''}${fmtNum(proj.trend_rate_pct)}%/mes`, MARGIN, doc.y, { width: CONTENT_W });
    doc.moveDown(0.8);
    ...
    scenarios.forEach((sc, i) => {
        ...
        doc.font('Helvetica-Bold').fontSize(9).fillColor(C.white)
            .text(`${sc.label.toUpperCase()}  ${sc.rate_pct >= 0 ? '+' : ''}${fmtNum(sc.rate_pct)}%/mes`, x + 8, startY + 7, { width: cardW - 16 });
        const rows = [
    ...
    if (data.cac_overall > 0 && data.summary.ltv_avg > 0) {
        ...
        doc.font('Helvetica').fontSize(8.5).fillColor(C.dark)
            .text(`Se o LTV/CAC sair de ${fmtNum(data.ltv_cac_overall ?? 0)}x para 3x: o negocio recuperaria o investimento em aquisicao em ${fmtNum((data.cac_overall / (data.summary.ltv_avg / 12)), 0)} meses.`, MARGIN, doc.y, { width: CONTENT_W });
        doc.y += 16;
        if (paybackImpact && paybackImpact > 12) {
            doc.font('Helvetica').fontSize(8.5).fillColor(C.warning)
                .text(`Atencao: payback atual de ${fmtNum(paybackImpact)} meses indica necessidade de aumentar LTV ou reduzir CAC.`, MARGIN, doc.y, { width: CONTENT_W });
            doc.y += 16;
        }
    }
    if (data.summary.refund_rate > 3) {
        ...
        doc.font('Helvetica').fontSize(8.5).fillColor(C.dark)
            .text(`Reduzir a taxa de reembolso de ${fmtPct(data.summary.refund_rate)} para 3% recuperaria aprox. ${fmtBrl(currentRefundImpact * 0.5)}/mes em receita.`, MARGIN, doc.y, { width: CONTENT_W });
        doc.y += 16;
    }
}
// ── Secao 7 — Bloco Fundraising ────────────────────────────────────────────────
function drawSection7(doc, data, profileName, periodLabel) {
    ...
    doc.font('Helvetica-Bold').fontSize(8).fillColor(C.primary)
        .text('Metricas formatadas para apresentacao a investidores. Para data room completo, acesse Northie Raise na plataforma.', MARGIN + 10, doc.y + 9, { width: CONTENT_W - 14 });
    doc.y += 38;
    const ltv_cac = data.ltv_cac_overall;
    ...
    const growthMoM = data.revenue_trend.length >= 2
        ...
        : null;
    const investorCols = [
        { label: 'METRICA', width: 0.30, align: 'left', font: 'Helvetica-Bold' },
        { label: 'VALOR', width: 0.28, align: 'right', font: 'Courier' },
        { label: 'VARIACAO / NOTA', width: 0.25, align: 'left' },
        { label: 'BENCHMARK', width: 0.17, align: 'left' },
    ];
    const investorRows = [
        {
            label: 'ARR Projetado',
...
function drawSection8(doc, data, ai, profileName, periodLabel) {
    drawSectionHeader(doc, '08', 'DIAGNOSTICO COMPLETO POR SEVERIDADE', profileName, periodLabel);
    if (!ai || ai.diagnosticos.length === 0) {
        doc.font('Helvetica').fontSize(9).fillColor(C.textSecondary)
            .text('Analise de IA nao disponivel neste relatorio. Gere o relatorio completo (PDF com IA) para visualizar o diagnostico.', MARGIN, doc.y, { width: CONTENT_W });
        return;
    }
    const order = ['critica', 'alta', 'media', 'ok'];
    const groupLabels = {
...
function drawSection9(doc, ai, data, profileName, periodLabel) {
    drawSectionHeader(doc, '09', 'PLANO DE ACAO PRIORIZADO', profileName, periodLabel);
    const steps = ai?.proximos_passos ?? [];
    if (steps.length === 0) {
        doc.font('Helvetica').fontSize(9).fillColor(C.textSecondary)
            .text('Gere o relatorio com analise de IA para receber o plano de acao personalizado.', MARGIN, doc.y, { width: CONTENT_W });
        return;
    }
    // Build action table with impact categorization
    const cols = [
        { label: 'PRIOR.', width: 0.08, align: 'center', font: 'Helvetica-Bold' },
        { label: 'ACAO', width: 0.54, align: 'left' },
        { label: 'AREA', width: 0.20, align: 'left' },
        { label: 'PRAZO', width: 0.18, align: 'center' },
    ];
    const areaMap = (step) => {
        if (/camp|ad|roas|meta|google|cpc|cpl/i.test(step))
            return 'Growth / Ads';
        if (/churn|retencao|reativac|cliente/i.test(step))
            return 'Retencao';
        if (/reembolso|produto|entrega/i.test(step))
            return 'Produto';
        if (/receita|faturamento|escal/i.test(step))
            return 'Financeiro';
        return 'Operacao';
    };
    const prazoMap = (step) => {
        if (/imediato|urgente|esta semana|hoje/i.test(step))
            return 'Imediato';
        if (/este mes|30 dias/i.test(step))
            return 'Este mes';
        return '30-60 dias';
    };
    ...
    doc.moveDown(0.3);
    doc.font('Helvetica').fontSize(7.5).fillColor(C.textSecondary)
        .text('Impactos estimados baseados nos dados do periodo. Execute em ordem de prioridade para maximizar resultado.', MARGIN, doc.y, { width: CONTENT_W });
}
// ── Secao 10 — Glossario ──────────────────────────────────────────────────────
function drawSection10(doc, profileName, periodLabel) {
...
export async function generatePdf(data, ai) {
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc.font('Helvetica').fontSize(7).fillColor(C.textSecondary)
            .text(`Gerado pela Northie em ${today} | northie.com.br | Confidencial`, MARGIN, footerY, { lineBreak: false, baseline: 'bottom' });
        doc.font('Helvetica').fontSize(7).fillColor(C.textSecondary)
            .text(`Pagina ${pageNum} de ${totalPages}`, 0, footerY, { width: PAGE_W - MARGIN, align: 'right', lineBreak: false, baseline: 'bottom' });
    }
    doc.end();
