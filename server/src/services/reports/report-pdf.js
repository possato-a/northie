...
function buildAnaliseIA(data, ai, profile, period, genDate) {
    ...
    const intStatusHtml = INTEGRATIONS_ALL.map(p => `<span style="display:inline-flex;align-items:center;gap:5px;margin:3px 6px 3px 0;font-size:11px;color:${active.has(p) ? '#22C55E' : '#9CA3AF'}">
           ${active.has(p) ? '●' : '○'} ${esc(plat(p))}
         </span>`).join('');
    // Top 3 métricas que precisam atenção
    const attentionItems = [];
    if (hs.breakdown && hs.breakdown.ltv_cac?.score < 50)
        attentionItems.push(`LTV/CAC abaixo do benchmark (score: ${hs.breakdown.ltv_cac?.score ?? '—'})`);
    if (hs.breakdown && hs.breakdown.refund?.score < 50)
        attentionItems.push(`Taxa de reembolso elevada (score: ${hs.breakdown.refund?.score ?? '—'})`);
    if (hs.breakdown && hs.breakdown.roas?.score < 50)
        attentionItems.push(`ROAS abaixo do esperado (score: ${hs.breakdown.roas?.score ?? '—'})`);
    if (hs.breakdown && hs.breakdown.growth?.score < 50)
        attentionItems.push(`Crescimento de receita lento (score: ${hs.breakdown.growth?.score ?? '—'})`);
...
function buildCanais(data, ai, profile, period, genDate) {
    ...
    function chanBadge(e) {
        if (e.status === 'lucrativo')
            return `<span class="badge-green">Lucrativo</span>`;
        if (e.status === 'organico')
            return `<span class="badge-blue">Orgânico</span>`;
...
function buildProjecoes(data, profile, period, genDate) {
    ...
    function projCard(cls, lbl, clr, p) {
    ...
    function scenarioBar(val, clr, lbl) {
...
function buildCohort(data, profile, period, genDate) {
    const cohortData = data.cohort ?? data.retencao ?? [];
    function cellClr(pct) {
        if (pct === null)
            return 'cohort-cell-empty';
        if (pct >= 70)
  ...
  ${pageFooter(10, genDate)}
    ...
    const m1Values = cohortData.map(c => c.m1).filter((v) => v !== null);
...
function buildValuation(data, profile, period, genDate) {
    const s = data.summary;
    const hs = data.health_score;
    ...
    function bmRow(label, value, benchmark, ok) {
        return `<tr>
    ...
    function radarBar(label, score, current, bench) {
...
function buildTopClientes(data, profile, period, genDate) {
    const s = data.summary;
