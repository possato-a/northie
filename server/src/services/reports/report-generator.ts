...
export async function generateReportData(profileId: string, frequency: ReportFrequency) {
    ...
    return {
        ...
        business_type: (profileData as { business_type?: string | null } | null)?.business_type ?? null,
        summary: {
            revenue_net: revenue,
...
export function formatAsCsv(
    ...
    return [
        ...
        ...Object.entries(revenue_by_platform).map(([k, v]) => `${k};${fmtBrl(v as number)}`),
        '',
        '=== INVESTIMENTO EM ADS ===',
        'Plataforma;Gasto',
        ...Object.entries(spend_by_platform).map(([k, v]) => `${k};${fmtBrl(v as number)}`),
        ...aiLines,
    ].join('\n');
}
