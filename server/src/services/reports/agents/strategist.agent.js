...
const BENCHMARKS = {
    saas: 'LTV/CAC > 3x (regra de ouro) | Churn mensal aceitável < 2% (crítico > 5%) | Reembolso < 3% | MRR growth saudável > 10%/mês',
    ecommerce: 'ROAS saudável 3-5x (crítico < 2x) | Reembolso < 5% (alto > 8%) | LTV/CAC > 2x | Repeat purchase > 30%',
    dtc: 'ROAS 3-6x (crítico < 2.5x) | Reembolso < 5% | LTV/CAC > 3x | Recompra > 25%',
    infoprodutor_perpetuo: 'ROAS > 3x | Reembolso < 7% | LTV/CAC > 2x | Recompra esperada > 20%',
    infoprodutor_lancamento: 'ROAS > 5x durante lançamento | Reembolso < 10% | LTV/CAC > 1.5x | Foco em volume de novos clientes',
    startup: 'Foco em crescimento — CAC alto é aceitável | LTV/CAC > 3x no longo prazo | Payback < 12 meses | Reembolso < 3%',
};
// ── Prompt ────────────────────────────────────────────────────────────────────
function buildPrompt(data, analystFindings) {
    ...
    const channelLines = data.channel_economics
        ...
        .map(c => `  ${c.channel}: status=${c.status} | ${c.new_customers} clientes | LTV ${fmtBrl(c.avg_ltv)} | CAC ${c.cac > 0 ? fmtBrl(c.cac) : 'orgânico'} | LTV/CAC ${c.ltv_cac_ratio ? fmt(c.ltv_cac_ratio) : '-'}`)
        .join('\n');
...
// ── Parser ────────────────────────────────────────────────────────────────────
const VALID_SITUACAO = ['saudavel', 'atencao', 'critica'];
const VALID_SEV = ['critica', 'alta', 'media', 'ok'];
const VALID_PRAZO = ['imediato', 'esta_semana', 'este_mes'];
function parseOutput(raw) {
    ...
    const diagnosticos = Array.isArray(parsed.diagnosticos)
        ? parsed.diagnosticos
            .filter((d) => d && typeof d === 'object')
            .map((d) => ({
            canal: String(d.canal ?? ''),
            severidade: VALID_SEV.includes(d.severidade) ? d.severidade : 'media',
            ...
            prazo: VALID_PRAZO.includes(d.prazo) ? d.prazo : 'este_mes',
        }))
        : [];
    return {
...
// ── Agent 2 ───────────────────────────────────────────────────────────────────
export async function runStrategistAgent(anthropic, data, analystFindings) {
