...
async function getCorrelationsContext(userId) {
    const lines = [];
    const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
    ...
    if (custError) {
        ...
        for (const c of custData) {
            const ch = c.acquisition_channel || 'desconhecido';
            if (!byChannel[ch])
                byChannel[ch] = { sum: 0, count: 0, champions: 0 };
            byChannel[ch].sum += Number(c.total_ltv) || 0;
            byChannel[ch].count += 1;
            if (c.rfm_score === 'Champions' || c.rfm_score === '555')
                byChannel[ch].champions += 1;
        }
        const sorted = Object.entries(byChannel).sort((a, b) => {
            const avgA = a[1].count > 0 ? a[1].sum / a[1].count : 0;
            const avgB = b[1].count > 0 ? b[1].sum / b[1].count : 0;
    ...
    if (txError) {
        ...
        for (const t of txData) {
            const p = t.platform || 'desconhecido';
            if (!byPlatform[p])
                byPlatform[p] = { net: 0, customers: new Set() };
            byPlatform[p].net += Number(t.amount_net) || 0;
            if (t.customer_id)
                byPlatform[p].customers.add(t.customer_id);
        }
...
async function getForecastContext(userId) {
    const lines = [];
    ...
    for (const t of txData) {
        ...
        const month = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        if (!byMonth[month])
            byMonth[month] = { net: 0, customers: new Set() };
        byMonth[month].net += Number(t.amount_net) || 0;
        if (t.customer_id)
            byMonth[month].customers.add(t.customer_id);
    }
...
async function getAnomaliesContext(userId) {
    const lines = [];
    const now = Date.now();
