...
export async function exportReport(req, res) {
    ...
    const periodFreqMap = { '30d': 'monthly', '90d': 'quarterly' };
    const frequency = periodFreqMap[period] ?? 'monthly';
    ...
    try {
        ...
        if (format === 'xlsx') {
            ...
            const dummyAi = { situacao_geral: 'atencao', resumo_executivo: '', diagnosticos: [], proximos_passos: [], generated_at: new Date().toISOString(), model: 'n/a' };
...
export async function sendReportByEmail(req, res) {
    ...
    const freqRaw = req.body.frequency ?? 'monthly';
    const frequency = FREQ_MAP[freqRaw] ?? 'monthly';
    const formatRaw = req.body.format ?? 'pdf';
    ...
    if (Array.isArray(req.body.recipients) && req.body.recipients.length > 0) {
        emailList.push(...req.body.recipients);
    }
    ...
    if (!emailList.length) {
        ...
        if (savedEmail) {
            const parsed = savedEmail.split(',').map((e) => e.trim()).filter(Boolean);
    ...
    try {
        ...
        const toField = emailList.length === 1 ? emailList[0] : emailList;
