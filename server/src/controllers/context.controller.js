...
export async function saveContext(req, res) {
    ...
    if (!profileId)
        return res.status(400).json({ error: 'Missing profileId' });
    const { segmento, icp, ticket_medio, ciclo_vendas, sazonalidades, instrucoes_ia, custom_fields, } = req.body;
    try {
        const payload = {
        ...
        if (icp !== undefined)
            payload.icp = icp;
        if (ticket_medio !== undefined)
            payload.ticket_medio = ticket_medio;
        if (ciclo_vendas !== undefined)
            payload.ciclo_vendas = ciclo_vendas;
        if (sazonalidades !== undefined)
            payload.sazonalidades = sazonalidades;
        if (instrucoes_ia !== undefined)
            payload.instrucoes_ia = instrucoes_ia;
