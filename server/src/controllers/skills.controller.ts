...
export async function listSkills(req: Request, res: Response) {
  const profileId = res.locals.profileId as string;
...
export async function updateSkill(req: Request, res: Response) {
  ...
  if (error) return res.status(500).json({ error: error.message });
  if (!data) return res.status(404).json({ error: 'Skill not found or not owned by you' });
  return res.json(data);
}

export async function toggleSkill(req: Request, res: Response) {
  const profileId = res.locals.profileId as string;
  const { id } = req.params;
  ...
  if (current.is_global) return res.status(403).json({ error: 'Cannot toggle global skills' });
  if (current.profile_id !== profileId) return res.status(403).json({ error: 'Not your skill' });
...
export async function deleteSkill(req: Request, res: Response) {
  ...
  if (error) return res.status(500).json({ error: error.message });
  if (count === 0) return res.status(404).json({ error: 'Skill not found or not deletable' });
