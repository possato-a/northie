...
type ResendEmailStatus = 'sent' | 'delivered' | 'bounced' | 'complained' | 'delayed';

const EVENT_TO_STATUS: Record<string, ResendEmailStatus> = {
    'email.sent':              'sent',
...
interface ResendEvent {
    ...
    data: {
        email_id: string;
        to: string[];           // array — usar data.to[0]
        from: string;
        subject?: string;
        created_at: string;
        [k: string]: unknown;
...
export async function handleResendWebhook(req: Request, res: Response) {
    ...
    try {
        event = JSON.parse(rawBody?.toString('utf8') ?? req.body.toString()) as ResendEvent;
    } catch {
