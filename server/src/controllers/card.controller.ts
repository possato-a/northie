 ...
 */
export async function getCardApplication(req: Request, res: Response) {
 ...
 */
export async function requestCard(req: Request, res: Response) {
    const profileId = req.headers['x-profile-id'] as string;
    if (!profileId) return res.status(400).json({ error: 'Missing x-profile-id header' });
    ...
    const splitRateByTerm: Record<number, number> = { 6: 0.18, 12: 0.12, 18: 0.09, 24: 0.07 };
    ...
    try {
        ...
        const notes = purposes?.length
            ? `Finalidades: ${(purposes as string[]).join(', ')}. Prazo: ${term_months} meses.`
