 ...
 */
export async function getPixelStats(req, res) {
    const profileId = req.headers['x-profile-id'];
    if (!profileId)
        return res.status(400).json({ error: 'Missing x-profile-id header' });
    try {
        ...
        const visits_last_7_days = visits.filter((v) => v.created_at && new Date(v.created_at) >= sevenDaysAgo).length;
        // Top fontes (utm_source ou 'Direto')
        const sourceCounts = {};
        for (const v of visits) {
            const source = v.utm_source || 'Direto';
