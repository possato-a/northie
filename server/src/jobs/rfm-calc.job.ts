 ...
 */
function quintileScore(value: number, sorted: number[], ascending = true): number {
    ...
    if (pct <= 0.2) return ascending ? 1 : 5;
    if (pct <= 0.4) return ascending ? 2 : 4;
    if (pct <= 0.6) return 3;
    if (pct <= 0.8) return ascending ? 4 : 2;
 ...
 */
async function buildChannelCacMap(
    profileId: string
 ...
 */
function churnProbability(daysSinceLastPurchase: number, frequency: number): number {
...
async function calcRfmForProfile(profileId: string): Promise<void> {
...
async function runRfmForAllProfiles(): Promise<void> {
