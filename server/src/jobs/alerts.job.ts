...
async function sendAlertEmail(to: string, title: string, body: string, severity: AlertSeverity): Promise<void> {
...
async function checkRoasDrop(profileId: string, notifyEmail?: string): Promise<void> {
...
async function checkHighChurn(profileId: string, notifyEmail?: string): Promise<void> {
...
async function checkRevenueZero(profileId: string, notifyEmail?: string): Promise<void> {
    const today = new Date().toISOString().split('T')[0]!;
    const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0]!;
...
async function checkOrganicSpike(profileId: string, notifyEmail?: string): Promise<void> {
...
async function checkHighCac(profileId: string, notifyEmail?: string): Promise<void> {
    ...
    if (!ltvData?.length || ltvData.length < 5) return;
    ...
    if (!newCustomers || newCustomers === 0) return;
...
async function checkBudgetSpike(profileId: string, notifyEmail?: string): Promise<void> {
    const today = new Date().toISOString().split('T')[0]!;
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]!;
    const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0]!;
    ...
    if (uniqueDays < 7) return; // dados insuficientes
