...
async function getClientCredentialsToken() {
    ...
    const res = await axios.post(HOTMART_AUTH_URL, new URLSearchParams({ grant_type: 'client_credentials' }), {
        headers: { Authorization: `Basic ${credentials}`, 'Content-Type': 'application/x-www-form-urlencoded' },
        timeout: 15000,
    });
    const token = res.data?.access_token;
    if (!token)
        throw new Error('[SafetyNet] Hotmart não retornou access_token');
    return token;
}
// ── API count check (1 request, sem paginação) ────────────────────────────
async function getHotmartApiCount(accessToken, startMs, endMs) {
...
// ── DB count check ────────────────────────────────────────────────────────
async function getDbCount(profileId, startIso, endIso) {
...
// ── Sync log ──────────────────────────────────────────────────────────────
async function logResult(profileId, apiCount, dbCount, backfillTriggered, gapFilled, error) {
...
async function reconcileProfile(profileId, accessToken) {
    const DAYS = 30;
...
async function reconcileShopifyProfile(profileId) {
    const DAYS = 30;
 ...
 */
async function reconcileStripeProfile(profileId) {
    const DAYS = 30;
    let synced = 0;
    try {
...
// ── Job starter ───────────────────────────────────────────────────────────
export function startSafetyNetJob() {
...
export { runSafetyNet };
//# sourceMappingURL=safety-net.job.js.map