...
async function refreshCorrelationViews(): Promise<void> {
...
async function upsertMonthlySnapshots(): Promise<void> {
...
function scheduleAt(hour: number, minute: number, dayOfMonth: number | null, fn: () => Promise<void>): void {
    let lastRun = '';
    const tick = () => {
        const now = new Date();
...
export function startCorrelationRefreshJob(): void {
