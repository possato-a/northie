...
async function getClientCredentialsToken() {
    ...
    const res = await withRetry(() => axios.post(HOTMART_AUTH_URL, new URLSearchParams({ grant_type: 'client_credentials' }), {
...
async function fetchAllHotmartSales(accessToken, startDateMs, endDateMs) {
    ...
    do {
        ...
        const res = await withRetry(() => axios.get(`${HOTMART_API_BASE}/payments/api/v1/sales/history`, {
            headers: {
 ...
 */
export async function backfillHotmart(profileId, days, force = false) {
        ...
        for (const sale of sales) {
            try {
                ...
                supabase
                    ...
                    .then(({ error }) => {
                    if (error)
                        console.warn(`[HotmartSync] platforms_data_raw insert failed for ${sale.transaction}:`, error.message);
                });
            }
            catch (e) {
 ...
 */
export async function runHotmartSyncForAllProfiles() {
