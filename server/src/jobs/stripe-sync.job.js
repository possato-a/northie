 ...
 */
function getStripeClient(connectedAccountKey) {
 ...
 */
async function processPaymentIntent(profileId, pi) {
    try {
        ...
        const email = pi.receipt_email || 'unknown@stripe.com';
        const name = '';
        // Upsert customer
        ...
        if (txError) {
            if (txError.code === '23505')
                return 'skipped'; // unique constraint — já existe
 ...
 */
async function processRefund(profileId, refund) {
 ...
 */
export async function backfillStripe(profileId, days) {
    ...
    const { access_token, stripe_user_id } = integration;
    if (!access_token) {
 ...
 */
export async function runStripeSyncForAllProfiles() {
...
//# sourceMappingURL=stripe-sync.job.js.map