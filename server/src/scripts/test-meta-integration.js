...
try {
    ...
    const appTokenResponse = await axios.get(`https://graph.facebook.com/oauth/access_token?client_id=${appId}&client_secret=${appSecret}&grant_type=client_credentials`, { timeout: 10000 });
...
if (!activeIntegrations || activeIntegrations.length === 0) {
    ...
    try {
        ...
        if (!accessToken) {
            ...
            const appId = process.env.META_APP_ID;
            const appSecret = process.env.META_APP_SECRET;
            const debugRes = await axios.get(`https://graph.facebook.com/debug_token?input_token=${accessToken}&access_token=${appId}|${appSecret}`, { timeout: 10000 });
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-meta-integration.js.map