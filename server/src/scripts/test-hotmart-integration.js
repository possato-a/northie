...
try {
    ...
    const res = await axios.post('https://api-sec-vlc.hotmart.com/security/oauth/token', new URLSearchParams({ grant_type: 'client_credentials' }), {
...
if (!connectToken) {
    ...
    try {
        ...
        const salesRes = await axios.get('https://api-hot-connect.hotmart.com/payments/api/v1/sales/history', {
        ...
        if (status === 401 || (typeof msg === 'string' && msg.includes('Decode token error'))) {
            warn('Connect API — configuração pendente', 'Credenciais precisam de permissão "Sales API" no painel Hotmart → Ferramentas → Developers. ' +
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-hotmart-integration.js.map