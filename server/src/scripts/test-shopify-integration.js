...
if (intError) {
    ...
    for (const int of integrations) {
        const lastSync = int.last_sync_at
            ...
            : 'nunca';
        const shopDomain = int.shopify_shop_domain || 'não definido';
        ...
        int.shopify_shop_domain
            ? ok(`shop_domain salvo`, int.shopify_shop_domain)
...
if (!activeIntegrations || activeIntegrations.length === 0) {
    ...
    try {
        ...
        shopDomain = activeIntegrations[0].shopify_shop_domain ?? null;
...
if (!shopToken || !shopDomain) {
    ...
    try {
        ...
        const wrongEndpoint = webhooks.filter((w) => !w.address.includes(backendUrl));
        wrongEndpoint.length === 0
            ? ok('Todos os webhooks apontam para o backend correto')
            : warn(`${wrongEndpoint.length} webhook(s) com endpoint desatualizado`, wrongEndpoint.map((w) => w.address).join(', '));
    }
    catch (err) {
...
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-shopify-integration.js.map